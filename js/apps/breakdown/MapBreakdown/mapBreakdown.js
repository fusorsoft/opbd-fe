  ...
  .directive('mapBreakdown', ['Maps', function (Maps) {
    var link = function (scope, elem, attrs) {
      ...
      var total = function (field) {
        return scope.selectedMapData.data.reduce(
          (a, b) => { return a + b[field] }, 0
        )
      }
      ...
      var updateData = function (map) {
        ...
        var wins = scope.selectedMapData.data.filter((m) => {
          return m.result === 'Win'
        }).length
        var losses = scope.selectedMapData.data.filter((m) => {
          return m.result === 'Loss'
        }).length
        var ties = scope.selectedMapData.data.filter((m) => {
          return m.result === 'Draw'
        }).length

        scope.differential =
          scope.selectedMapData.historyData.slice(-1)[0].differential
      ...
      scope.mostPlayedMaps =
        sortedByTimesPlayed.slice(-7).reverse().map(getMapData)
      scope.otherMaps = arrMaps.length > 7
        ? sortedByTimesPlayed.slice(0, arrMaps.length - 7)
          .reverse()
          .map(getMapData)
        : null
