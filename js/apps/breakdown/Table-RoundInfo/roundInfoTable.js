...
breakdownDirectives.directive('roundInfoTable', function() {
...
breakdownDirectives.directive('roundSummaryRow', function() {
...
breakdownDirectives.directive('roundDetailRow', ['Weapons', function(Weapons) {
	...
	var link = function(scope, elem, attrs) {
			...
			try {
				if (d.Weapon !== 'Unknown') {
					...
					if (d.Position.Weapon === "Incendiary") {
						// this needs to be investigated more thoroughly, but this appears to be what
						// it looks like when you kill someone with fire...
						weapon = 'Fire';
					}
				}
		...
		if (scope.roundData.KillData && scope.roundData.KillData.length > 0) {
			for(var i = 0; i < scope.roundData.KillData.length; i++) {
				scope.killFeed.push(extractData(scope.roundData.KillData[i], true, scope.roundData.Team));
			}
		}

		if (scope.roundData.DeathInfo) {
			scope.killFeed.push(extractData(scope.roundData.DeathInfo, false, scope.roundData.Team));
		}
