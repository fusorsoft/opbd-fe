		...
		$scope.removeFriend = function(friendId) {
			...
			userData.removeFriend(userId, friendId)
				.then(function() {
					...
					toaster.pop('success', 'Success', "Removed " + friendName + " from your friends list");
				},
				function(err) {
					toaster.pop('error', 'Error', "Unable to remove " + friendName + " from friends list");
				});
		};
					...
					$q.all(matchPromises).then(function(friendMatchSummaries) {
						...
						for (friend in $scope.friends) {
							...
							for (var i = currentIndex - 1; i > 0; i--) {
								if (matchUtils.getMatchResult(thisFriend.matches[i]) === lastResult) {
									count++;
								} else {
									break;
								}
